// config.js — the shared configuration room.
//
...
// a Matrix room that EVERY user can read. The admin surface (admin.html) writes
// to this room; each user's app folds it to learn the current document.
//
...
//   answerRoomFor(user)      private — one per user. Their answers + message
//                            content live here, never mixed with anyone else's.
//
...
//   DEF anchor="config"     path "field_order" -> [id,...]   (ordering, last write wins)
...
export const CONFIG_ANCHOR = "config";
...
// person's message content is never visible in another's timeline.
