...
function showLoader() {
  ...
  $("loadName").textContent = "Preparing SmolLM2 360M (q4)…";
...
function updateLoader(text, p) {
  ...
  $("loadName").textContent = pct >= 100 ? "Almost ready — finishing up…" : "Downloading SmolLM2 360M (q4)";
...
async function boot() {
  ...
  if (modelKind === "webllm") {
    ...
    try {
      ...
      hideLoader();
      setStatus("SmolLM2 360M q4 · ready", "live");
    } catch (e) {
      hideLoader();
